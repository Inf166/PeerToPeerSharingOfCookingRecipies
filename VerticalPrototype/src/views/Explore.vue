<template>
  <div class="explore">
    ...
    <div class="previews" v-if="suggestedRecipes.length">
      <h3>Rezepte die dir gefallen könnten</h3>
      <div class="recipe-teaser" v-for="(recipe, index) in suggestedRecipes" :key="'suggestedRecipes+'+index">
        ...
        <div class="recipe-teaser-buttons">
          <router-link :to="{name: 'Recipe', params: { showRecipe: recipe }}" tag="button" class="primary-button" >
    ...
    <div class="previews">
      ...
      <div class="recipe-teaser" v-for="(recipe, index) in friendRecipes" :key="'FriendRecipes+'+index">
        <div class="recipe-teaser-img">
          <img src="https://via.placeholder.com/1920x1080.png" alt="Rezept2" >
        </div>
        <div class="recipe-teaser-desc">
...
export default {
  computed: {
    ...
    suggestedRecipes() {
      return this.$store.getters.prefrencedRecipes;
    }
  },
  methods: {
    refreshPeers() {
      ...
      for(var friend in this.myFriends) {
        peer.methods.fetchJoin(this.myPeer, this.users[this.myFriends[friend]].peerID).then((data) => {
          console.log("Got Recipes from peer: ");
          console.log(data);
          data.forEach(element => {
            document.getElementById("refresh-icon").classList.remove("spinnin");
            this.$store.dispatch("addFriendRecipes", element);
          });
    ...
    removeRecipeFromFriendRecipes(index) {
